...
print("="*70)
print("ðŸ‘ï¸  Testing ANA Vision Tools")
print("="*70)
...
try:
    ...
    if result.is_success:
        ...
        print(f"  âœ… SUCCESS! Screenshot saved: {screenshot_path}")
    else:
...
try:
    ...
    if result.is_success:
        ...
        print(f"  âœ… SUCCESS! Found {controls} UI controls")
...
try:
    ...
    if cap_result.is_success:
        ...
        if result.get('status') == 'success':
            ...
            print(f"  âœ… SUCCESS! OCR found text: {text}...")
        else:
            print(f"  âŒ FAILED: {result.get('error')}")
    else:
        print(f"  â­ï¸  SKIPPED (capture failed)")
except Exception as e:
    print(f"  âŒ ERROR: {e}")

print("\n" + "="*70)
print("âœ… Vision Tools Test Complete")
print("="*70)
