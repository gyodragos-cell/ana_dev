...
class BugAnalyzer:
    ...
    def check_corrupted_files(self):
        ...
        print("ðŸ“‹ Checking for corrupted files...")
            ...
            try:
                with open(py_file, 'r', encoding='utf-8') as f:
                    ...
                    if '\x00' in content:
                        ...
                        print(f"  âŒ {py_file.name} - CORRUPTED")
                    # Try to parse
                    try:
                        ast.parse(content)
                    except SyntaxError as e:
                        self.warnings.append(f"Syntax error in {py_file}: {e}")
                        print(f"  âš ï¸  {py_file.name} - SYNTAX ERROR")
    ...
    def check_tool_definitions(self):
        ...
        print(f"  âœ… {tool_count} tools validated\n")

    def check_jules_integration(self):
        """Check Jules integration"""
        print("ðŸŽ¯ Checking Jules integration...")
    ...
    def check_memory_leaks(self):
        ...
        if memory_dir.exists():
            ...
            print(f"  âœ… {len(db_files)} database files ({total_size/1024:.1f} KB)")
        else:
            self.warnings.append("Memory directory not found")
            print(f"  âš ï¸  Memory directory missing")
        ...
        if logs_dir.exists():
            ...
            if log_file.exists():
                ...
                if size > 50 * 1024 * 1024:  # 50MB
                    ...
                    print(f"  âš ï¸  Log file large: {size/1024/1024:.1f} MB")
                else:
                    print(f"  âœ… Log file OK ({size/1024:.1f} KB)")
        ...
        if env_file.exists():
            with open(env_file) as f:
                ...
                if 'your_key_here' in content or 'your_secret' in content:
                    ...
                    print(f"  âš ï¸  .env has placeholder keys")
                else:
                    print(f"  âœ… .env configured")
        else:
    ...
    def print_report(self):
        ...
        if self.errors:
            print(f"âŒ ERRORS ({len(self.errors)}):")
        ...
        if self.fixed:
            print(f"âœ… FIXED ({len(self.fixed)}):")
