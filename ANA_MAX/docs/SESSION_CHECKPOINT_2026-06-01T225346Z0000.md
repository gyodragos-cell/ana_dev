# Session Checkpoint - 2026-06-01T22:53:46+00:00

## Code Map demotes generated memory noise

## Summary

Improved ana_code_map query ranking so generated memory paths and archives do not outrank active work for general project-state questions. SESSION_CHECKPOINT, rem_sleep, dev_artifacts archives, and ANA_MAX archives are demoted unless the query explicitly asks for checkpoint, REM, archive, or security-research material. This makes code_context_pack return ana_autonomy_runner.py as the top candidate for next scoped lab action after green baseline instead of stale generated history.

## Next Steps

- Continue with one new scoped lab action. Consider refreshing Code Map/Graph Map only when source summaries need updated content
- current ranking change works live without MCP restart.
...
```text
compileall ana_code_map.py/code_context_pack_tool.py PASS; focused pytest test_ana_code_map.py test_code_context_pack_tool.py test_ana_live_behavior_check.py PASS 19; review batch script PASS; review batch test PASS full tests/runtime; review batch doc PASS governance; real ana_code_map query no longer returns checkpoints/archives in top; MCP code_context_pack returns ana_autonomy_runner.py as top candidate; no_reload_quality_gate_20260602_015309 PASS 8/8; Operator Status reload/tool_surface/behavior/identity/memory PASS and review verified 6/6 fresh.
```

## Risks

- Large dirty tree remains expected lab state. Generated memory/archive content is demoted, not removed
- explicit checkpoint/archive/security queries still work.
