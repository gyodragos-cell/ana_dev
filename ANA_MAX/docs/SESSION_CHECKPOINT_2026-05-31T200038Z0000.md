# Session Checkpoint - 2026-05-31T20:00:38+00:00

## Patch Advisor dirty-tree evidence fix

## Summary

Fixed ana_patch_advisor so large_dirty_tree recommendations use Error Radar's full dirty-tree details instead of only sampled git paths. This prevents checkpoint/REM noise from being undercounted in advice evidence.

## Current Goal

Improve ANA self-healing diagnostics so recommendations are accurate, suggest-only, and calm.

## Next Steps

- Continue reliability/autonomy work
- keep patch_advisor suggest-only and rely on focused tests before edits.
...
```text
patch_advisor/autonomy pytest 21 passed; live patch advisor evidence now reports 205 checkpoints from Error Radar; lab_quality_gate PASS
```

## Risks

- Dirty tree remains large by design
- advisor still warns against blind commits/patches.
