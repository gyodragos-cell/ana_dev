# Session Checkpoint - 2026-05-31T23:18:24+00:00

## Reload runbook documents live behavior WARN exit

## Summary

Updated ANA_OPERATOR_RELOAD_RUNBOOK to clarify that ana_live_behavior_check.py returns non-zero while live behavior is WARN, so reload verification commands should be run separately instead of chained with &&. Verified reload diagnostics remain aligned and governance stays clean.
...
```text
reload readiness: WARN live_behavior_stale; reload consistency: PASS aligned=True readiness/operator/lab_state/post_reload all WARN; post reload verify: WARN with tool_surface/identity/nucleus PASS; live behavior: WARN expected; pytest governance: 4 passed; governance check: PASS 116/116
```
