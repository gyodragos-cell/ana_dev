# Session Checkpoint - 2026-05-31T11:19:30+00:00

## Autonomy Runner includes Patch Advisor

## Summary

Integrated suggest-only Patch Advisor into ana_autonomy_runner.py as an optional local pre-edit planning step. Live Autonomy now PASS 12/12 and includes patch_advisor after error_radar before session_audit, carrying blast-radius availability/affected signals.

## Current Goal

Make ANA's autonomy readiness loop anticipate patch impact and self-healing recommendations before Codex edits.

## Next Steps

- After MCP reload, verify graph_context_pack action=blast live
- consider adding agent_trace_schema spans to autonomy_runner steps next.
...
```text
pytest autonomy+patch focused PASS 16/16; live ana_autonomy_runner PASS 12 pass / 0 warn / 0 fail trust=86 report autonomy_runner_20260531_111632.json; governance PASS 111/111; no_reload_quality_gate PASS 7/7 report no_reload_quality_gate_20260531_141859.json.
```

## Risks

- Patch Advisor remains suggest-only and depends on current git/graph state
- live MCP still needs reload for newly added graph_context_pack action=blast.
