# Session Checkpoint - 2026-05-30T21:48:56+00:00

## Activity Bar lab quality gate command

## Summary

Added ANA MAX Lab Quality Gate as an Activity Bar and command-palette command only. It runs ANA_MAX/dev_artifacts/scripts/lab_quality_gate.py through the stable Live Console path and intentionally does not add any webview/cockpit button.

## Current Goal

Keep ANA MAX operator workflow reliable from the left Activity Bar while avoiding fragile webview controls.

## Next Steps

- Package/reload extension only when operator is ready
- Keep new controls Activity-Bar-first
- Consider adding a compact left-side Coverage command only if needed.
...
```text
node --check vscode_extension/extension.js passed; package.json validates; pytest test_vscode_extension 25 passed; lab_quality_gate PASS report ANA_MAX/dev_artifacts/reports/lab_quality_gate_20260530_214848.json
```

## Risks

- Installed extension needs package/install/reload before the new Activity Bar command appears. Webview remains unchanged by design.
