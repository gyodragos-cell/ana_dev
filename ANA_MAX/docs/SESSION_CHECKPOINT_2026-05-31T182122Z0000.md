# Session Checkpoint - 2026-05-31T18:21:22+00:00

## Reload runbook aligned to three-signal readiness

## Summary

Updated ANA_OPERATOR_RELOAD_RUNBOOK and NEXT_SESSION_BOOTSTRAP to reflect v1.0.47 installed state and three-signal reload readiness: marker, tool_surface, and live_behavior. Verified readiness now reports reload_needed=True for current live drift. Docs and tests align with v1.0.47.

## Current Goal

Keep the operator handoff accurate while VS Code reload and MCP restart are still pending.

## Next Steps

- Run Developer: Reload Window, restart ANA MCP, then run ANA MAX: Post-Reload Verify. Expected: tools=90, tool_surface PASS, behavior PASS, identity PASS.
...
```text
governance PASS 115/115; VSIX version PASS expected=1.0.47; focused reload/extension/vsix tests PASS 35/35; reload_readiness live reports reload_needed=True with reasons live_tool_surface_drift and live_behavior_stale; no_reload_quality_gate PASS 8/8 with expected live advisories; lab_quality_gate PASS 10/10.
```

## Risks

- Active VS Code window still needs manual reload
