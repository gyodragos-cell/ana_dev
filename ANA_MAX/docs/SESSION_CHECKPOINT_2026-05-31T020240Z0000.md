# Session Checkpoint - 2026-05-31T02:02:40+00:00

## Quality gate examples aligned with VSIX version guard

## Summary

Lab and no-reload quality gate examples are aligned with the vsix_version_consistency guard. Full lab_quality_gate PASS report lab_quality_gate_20260531_020120.json confirms compile, focused tests, governance, permission manifest, extension JS, VSIX version consistency, MCP health, and Nucleus Smoke all pass.
...
```text
Focused pytest PASS: tests/runtime/test_ana_vsix_version_check.py tests/runtime/test_no_reload_quality_gate.py tests/runtime/test_lab_quality_gate_contract.py tests/runtime/test_ana_governance_check.py; governance PASS 111/111; lab_quality_gate PASS report ANA_MAX/dev_artifacts/reports/lab_quality_gate_20260531_020120.json
```

## Risks

- Live MCP still needs reload before marker clears
- use local checkpoint fallback until MCP reload confirms marker=True.
