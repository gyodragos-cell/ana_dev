# Session Checkpoint - 2026-05-31T21:01:02+00:00

## Reload runbook split into MCP and VSIX lanes

## Summary

Updated ANA_OPERATOR_RELOAD_RUNBOOK with Lane A for Python-only live_behavior_stale MCP restart and Lane B for VSIX/VS Code surface reload. NEXT_SESSION_BOOTSTRAP now tells future sessions not to install VSIX or reload VS Code when marker/tool_surface are PASS and only live_behavior is stale.

## Current Goal

Make operator documentation match the precise reload diagnostics built in scripts.

## Next Steps

- Restart ANA MCP when convenient, then run Live Behavior, Reload Consistency, Operator Status, Post-Reload Verify, and Autonomy Pass.
...
```text
governance PASS 116/116; reload/operator focused pytest 25 passed; no_reload_quality_gate PASS with expected live_behavior advisory; lab_quality_gate PASS
```

## Risks

- Live MCP remains stale until restart
- docs now explain the direct restart lane.
