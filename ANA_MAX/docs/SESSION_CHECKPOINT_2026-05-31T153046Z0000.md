# Session Checkpoint - 2026-05-31T15:30:46+00:00

## Operator status surfaces live behavior drift

## Summary

Extended ana_lab_state_summary and ana_operator_status to include live behavior freshness as behavior=STATUS(checks=P/T). Current live status now shows tool_surface=WARN, behavior=WARN(checks=0/2), identity=PASS, making stale MCP behavior visible in the fast operator line.

## Current Goal

Keep ANA MAX operator status high-signal: online state, live tool surface, live behavior freshness, identity, trace, and file activity in one view.

## Next Steps

- Restart MCP when convenient, then rerun ana_post_reload_verify.py --no-write and ana_operator_status.py. Expected after restart: tool_surface PASS and behavior PASS.
...
```text
operator/lab-state/live-behavior pytest 17/17 PASS; no_reload_quality_gate PASS 8/8 with expected live advisories; lab_quality_gate PASS 10/10; live operator/lab status displays behavior=WARN(checks=0/2) and identity=PASS.
```

## Risks

- Live MCP still needs restart to clear stale behavior and old tool surface.
