# Session Checkpoint - 2026-05-31T11:04:12+00:00

## Agent trace schema foundation

## Summary

Added local OTel-inspired ANA agent trace span schema in ANA_MAX/core/agent_trace_schema.py plus docs/ANA_AGENT_TRACE_SCHEMA.md. Spans store run/trace/span identity, operation/status/risk, timing, input/result digests, sanitized evidence, and raw_private_payloads=false.

## Current Goal

Build ANA's private local reliability/autonomy layer so tool calls, context packs, verification, audit, and checkpoints can be chained safely.

## Next Steps

- Use agent_trace_schema in session_audit/autonomy_runner later
- implement Graph Map blast-radius query next
- restart MCP later to pick up tool_healthcheck dependency reporting.
...
```text
pytest test_agent_trace_schema PASS 4/4; governance PASS 111/111; no_reload_quality_gate PASS 7/7 report no_reload_quality_gate_20260531_140354.json.
```

## Risks

- Schema is foundation only
- not yet wired into all runtime event emitters. Keep raw payloads out when integrating.
