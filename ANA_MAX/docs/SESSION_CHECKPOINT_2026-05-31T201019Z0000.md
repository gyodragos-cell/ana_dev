# Session Checkpoint - 2026-05-31T20:10:19+00:00

## Error Radar runtime path classification

## Summary

Fixed error_radar dirty-tree breakdown so runtime paths are counted correctly when git status is run from inside ANA_MAX. Paths like tools/, core/, main.py, and dev_artifacts/scripts/ now count as runtime/script surface instead of being missed.

## Current Goal

Improve ANA diagnostics so dirty-tree findings distinguish memory/docs noise from active runtime change surface.

## Next Steps

- Restart MCP later if live error_radar still reports runtime=0
- disk-side tests and gates already cover the corrected behavior.
...
```text
error_radar/patch_advisor tests 15 passed; disk-side ErrorRadarTool reports runtime=65 for current dirty tree; lab_quality_gate PASS; no_reload_quality_gate PASS
```

## Risks

- Live MCP may continue using the old classification until server restart
- diagnostics should use reload checks if live/disk behavior matters.
