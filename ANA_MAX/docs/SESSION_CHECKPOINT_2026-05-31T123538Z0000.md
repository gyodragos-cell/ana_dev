# Session Checkpoint - 2026-05-31T12:35:38+00:00

## No-reload gate detects live tool-surface drift

## Summary

Added ana_live_tool_surface_check.py and wired it as a no-reload gate advisory so stale live MCP tools/list drift is visible without failing the main quality gate.

## Current Goal

Keep Codex-first active surface honest across disk/live MCP reload boundaries.

## Next Steps

- Restart ANA MCP when ready. Then run ana_live_tool_surface_check.py and ana_post_reload_verify.py --no-write
- expected target is PASS(live=90,manifest=90,extra=0,missing=0).
...
```text
live tool surface tests/no-reload/operator PASS 15/15; live surface check WARN expected extra_live=adal_integration; no_reload_quality_gate PASS 7/7 with live_tool_surface advisory; lab_quality_gate PASS 9/9.
```

## Risks

- Live MCP still exposes stale 91-tool surface until restart. Advisory is expected and non-blocking for disk-side work.
