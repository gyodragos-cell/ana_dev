# Session Checkpoint - 2026-05-31T01:14:41+00:00

## Lab VSIX install helper added

## Summary

Added install_latest_lab_vsix.ps1 as a guarded operator helper for installing the latest packaged ANA MAX lab VSIX. It is dry-run by default, reads the version from vscode_extension/package.json, requires the VSIX to exist, and only installs when -Apply is passed. Added LAB_VSIX_INSTALL_HELPER_EXAMPLE, tests, examples index entry, and governance requirement. Verified dry-run output prints code --install-extension for ana-codex-cockpit-1.0.44.vsix without installing; tests PASS 33/33, governance PASS 106/106, no_reload_quality_gate PASS 6/6 with expected live_reload_marker advisory WARN.
