# Session Checkpoint - 2026-06-01T22:40:27+00:00

## Live Behavior catches stale agent_coach monitor filter

## Summary

Extended ana_live_behavior_check.py with an agent_coach monitor-noise probe. Live Behavior now reports WARN 7/8 when live MCP still runs old coach loop detection even though disk-side coach filters read-only monitor/demo telemetry. Updated focused tests and operator docs for the new 8/8 clean shape.

## Next Steps

- Operator restart ANA MCP when ready
- run Live Behavior
- run Reload Consistency
- run Post-Reload Verify
- rerun Operator Status
- do not install VSIX or reload VS Code unless another signal requires it.
...
```text
compileall ana_live_behavior_check.py and agent_coach_tool.py PASS; pytest tests/runtime/test_ana_live_behavior_check.py tests/runtime/test_agent_coach_recommend.py PASS 12; Review Batch script PASS; Review Batch test PASS full tests/runtime; Review Batch doc PASS governance; Operator Status shows review verified 6/6 fresh and memory PASS; live behavior remains expected WARN 7/8 until MCP restart.
```

## Risks

- Current live MCP is intentionally stale for agent_coach_monitor_noise_filter until server restart. no_reload/lab gate reports remain older or WARN/FAIL because live MCP has not been restarted after this disk-side behavior change.
