# Session Checkpoint - 2026-05-31T20:46:24+00:00

## Autonomy stale behavior guidance aligned

## Summary

Aligned ana_autonomy_runner next_action for live_behavior WARN with the rest of reload diagnostics. It now says to restart ANA MCP, then run Live Behavior, Reload Consistency, Post-Reload Verify, and Autonomy Pass.

## Current Goal

Make every operator-facing stale-live diagnostic recommend the same precise follow-up sequence.
...
```text
autonomy/reload consistency focused pytest 16 passed; live Autonomy WARN only on live_behavior with aligned next_action; lab_quality_gate PASS; no_reload_quality_gate PASS with expected live_behavior advisory WARN
```

## Risks

- Live MCP remains stale until restart
- source code and tests are passing.
