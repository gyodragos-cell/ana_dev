# Session Checkpoint - 2026-05-31T18:40:33+00:00

## Autonomy Pass stale-live guard

## Summary

Updated ana_autonomy_runner.py to include live_tool_surface and live_behavior verification steps. Autonomy Pass now gives restart MCP priority when the live server has stale tool surface or stale selected behavior, even if the reload marker is present and coach recommends an action.

## Current Goal

Keep autonomy suggestive and evidence-first; do not let it proceed to action while live MCP is stale.

## Next Steps

- Manual operator step remains: Developer: Reload Window, restart ANA MCP, then run Post-Reload Verify and Autonomy Pass. Expected after restart: live_tool_surface PASS, live_behavior PASS, Autonomy can return to coach-driven next_action.
...
```text
focused autonomy/status tests PASS 27/27; live Autonomy Pass WARN with 13 pass / 2 warn / 0 fail and next_action restart MCP; no_reload_quality_gate PASS 8/8 with expected live advisories; lab_quality_gate PASS 10/10.
```

## Risks

- Current WARNs are expected until live MCP restarts and loads disk-side behavior.
