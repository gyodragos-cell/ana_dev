# Session Checkpoint - 2026-05-31T12:31:30+00:00

## Post-reload verify checks tool surface
...
Make disk-vs-live drift visible and actionable after Codex-first surface cleanup.

## Next Steps

- Restart ANA MCP when ready, then run ana_post_reload_verify.py --no-write
...
```text
post-reload/operator tests PASS 14/14; live post-reload verify WARN due expected stale tool_surface live=91 manifest=90 extra=1; lab_quality_gate PASS 9/9.
```

## Risks

- Current live MCP still needs operator restart to drop stale tool from tools/list. WARN is expected until restart.
...
?? ANA_MAX/data/events_stream.db-journal
