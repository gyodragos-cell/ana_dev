# Session Checkpoint - 2026-05-31T10:12:21+00:00

## Patch Advisor filters stale radar noise

## Summary

Improved ana_patch_advisor.py so it filters known debugger traceback_text smoke-test findings inherited from stale live MCP error_radar output. Report now tracks raw_finding_count, finding_count, and noise_filtered. Live advisor run filtered 2 raw findings and kept only the real large_dirty_tree recommendation.
...
- Advisor filters known noise locally, but live error_radar itself still needs MCP reload for the same filter in tool output.
