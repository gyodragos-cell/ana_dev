# Session Checkpoint - 2026-05-31T12:57:02+00:00

## Post-reload verify includes identity surface

## Summary

Extended ana_post_reload_verify to expose identity_surface in JSON and human output. Post-reload status now WARNs when active identity surface drifts, with next_action to fix identity first. Added post-reload verify script/tests to no_reload_quality_gate and lab_quality_gate coverage. Updated examples, master context, Codex manager prompt, and AGENT_MEMORY.

## Current Goal

Keep ANA MAX operator/reload flow compact, Codex-first, and evidence-driven.

## Next Steps

- After operator MCP restart, rerun ana_post_reload_verify.py --no-write and expect tool_surface PASS with identity PASS
- then continue autonomy/reliability work.
...
```text
post-reload/operator/lab-state pytest 18/18 PASS; post-reload contract pytest 9/9 PASS; no_reload_quality_gate PASS 8/8 with advisory live_tool_surface WARN; lab_quality_gate PASS 10/10; live post-reload verify WARN only due stale live tool surface, identity PASS.
```

## Risks

- Live MCP still needs restart to drop stale adal_integration from tools/list.
