# Session Checkpoint - 2026-05-31T12:21:04+00:00

## Codex-only active surface and AdaL retired

## Summary

Retired adal_integration from active tool registration, permission manifest, backend routing, and active docs. Updated extension copy to Codex-first and removed external-agent promotion wording from active lab guidance.

## Current Goal

Keep ANA MAX mother-lab focused on Codex-first local runtime quality, not multi-agent promotion.

## Next Steps

- Restart MCP when ready so live tools_count drops from 91 to 90
- continue adding guardrails that keep active docs/extension neutral and Codex-first.
...
- ANA_MAX/core/agent.py
- ANA_MAX/core/backends/router.py
- ANA_MAX/core/mcp_server.py
...
```text
permission manifest coverage PASS runtime=90 manifest=90; governance PASS 111/111 tools_total=90; focused extension/governance tests PASS 34/34; lab_quality_gate PASS 9/9.
```

## Risks

- Live MCP process may still show old tools_count=91 until server reload. Historical archives/reports still contain old external-tool names by design.
