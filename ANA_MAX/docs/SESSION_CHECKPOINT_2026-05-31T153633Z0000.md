# Session Checkpoint - 2026-05-31T15:36:33+00:00

## VSIX 1.0.47 Live Behavior Activity Bar command

## Summary

Added ANA MAX: Live Behavior to the VS Code Activity Bar and command palette. It runs ana_live_behavior_check.py from the Live Console path and surfaces healthy-but-stale MCP behavior directly in the operator UI. Bumped lab extension to v1.0.47, updated docs/version references, packaged VSIX artifacts, and verified install helper dry-run points to 1.0.47.

## Current Goal

Keep the stable Activity Bar surface aligned with the new observability checks so Billy and Codex can see live MCP freshness without using terminal commands manually.

## Next Steps

- Install v1.0.47 with install_latest_lab_vsix.ps1 -Apply when ready, reload VS Code, restart ANA MCP, then run Post-Reload Verify. Expected after restart: tool_surface PASS and behavior PASS.
...
```text
extension/vsix tests PASS 32/32; node --check PASS; package.json JSON PASS; ana_vsix_version_check PASS expected=1.0.47; package_cockpit_vsix built 1.0.47 artifacts; no_reload_quality_gate PASS 8/8 with expected live advisories; lab_quality_gate PASS 10/10; install helper dry-run points to 1.0.47; operator status reports vsix=1.0.47.
```

## Risks

- Live installed extension remains old until operator installs VSIX/reloads VS Code
- live MCP remains stale until restart.
