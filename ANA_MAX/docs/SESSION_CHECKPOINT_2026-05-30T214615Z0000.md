# Session Checkpoint - 2026-05-30T21:46:15+00:00

## Activity Bar profile status command

## Summary

Added ANA MAX Profile Status as a stable Activity Bar and command-palette command only. It calls tool_router mode=profile_status and displays JSON through runInCockpit response. Avoided adding cockpit/webview buttons because the operator reported webview is fragile and only the left Activity Bar works reliably.

## Current Goal

Keep ANA MAX operator controls on the stable left Activity Bar surface while avoiding fragile webview changes.

## Next Steps

- Restart/repackage extension only when operator is ready
- Keep future operator controls Activity-Bar-first
- Consider de-emphasizing cockpit webview further in docs/tests.
...
```text
node --check vscode_extension/extension.js passed; package.json validates; pytest test_vscode_extension 24 passed; lab_quality_gate PASS report ANA_MAX/dev_artifacts/reports/lab_quality_gate_20260530_214607.json
```

## Risks

- The installed VSIX/window will not show the new Activity Bar command until the extension is repackaged/installed/reloaded. Webview intentionally unchanged.
