# Session Checkpoint - 2026-06-01T17:12:57+00:00

## Surface first review command in Operator Status

## Summary

Operator Status now extracts the latest Autonomy Patch Advisor first_review_batch and first_review_commands, then prints a compact review=batch command=... line for the next focused verification step.

## Current Goal

Make ANA's self-healing recommendation visible directly in the operator surface, not only inside JSON reports.

## Next Steps

- Use Operator Status review command before broadening investigation
...
```text
pytest tests/runtime/test_ana_operator_status.py: 22 passed; compileall ana_operator_status.py passed; live Operator Status prints review=batch=runtime command=python -m compileall...; no_reload_quality_gate_20260601_201233 PASS 8/8; governance_check_20260601_171044 PASS 116/116.
```

## Risks

- No MCP restart needed because this is a local operator script/docs change
- latest live MCP remains unchanged.
