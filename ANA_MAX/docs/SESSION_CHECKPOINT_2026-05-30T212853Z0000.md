# Session Checkpoint - 2026-05-30T21:28:53+00:00

## Profile-aware router and coach polish

## Summary

Extended tool_router so recommendations honor permission_manifest active_profiles, include tool_profiles/active_profiles/filtered_by_profile in responses, and propagate those fields through agent_coach recommendations. Documented runtime profile enforcement in ANA_PROFILE_MANIFEST.

## Current Goal

Continue ANA MAX mother-lab stabilization toward profile-aware routing, safer tool recommendation, and verified governance.

## Next Steps

- Classify remaining permission_manifest tools with core/windows/linux/security_lab/private_lab profiles
- Consider exposing profile status in the VS Code Activity Bar or dashboard
- Restart MCP when ready so the live server picks up router/coach profile-aware code.
...
```text
pytest router/coach/governance 12 passed; ana_governance_check PASS 58/58; lab_quality_gate PASS report ANA_MAX/dev_artifacts/reports/lab_quality_gate_20260530_212734.json; python ANA_MAX/main.py --test loaded 91 tools and quick tests 2 PASS / 0 FAIL
```

## Risks

- The currently running MCP process may need restart before live MCP calls reflect the new router/coach code. Not every tool in permission_manifest has explicit profile metadata yet.
