# Session Checkpoint - 2026-06-01T23:00:48+00:00

## Graph Map demotes generated memory noise

## Summary

Aligned Graph Map query behavior with Code Map ranking. graph_context_pack now demotes keyword hubs and generated-memory/archive file nodes for general project-state questions, and hides low-signal checkpoint/archive neighbors unless the query explicitly asks for that material. This keeps graph_context_pack and code_context_pack focused on active files for next scoped lab action queries.
...
```text
compileall ana_graph_map.py/graph_context_pack_tool.py/code_context_pack_tool.py PASS; focused pytest test_ana_graph_map.py test_graph_context_pack_tool.py test_code_context_pack_tool.py PASS 14; review batch script PASS; review batch test PASS full tests/runtime; review batch doc PASS governance; live graph_context_pack query returns active files/symbols without checkpoint/archive keyword-neighbor noise; code_context_pack still returns ana_autonomy_runner.py as top candidate; no_reload_quality_gate_20260602_020019 PASS 8/8; Operator Status reload/tool_surface/behavior/identity/memory PASS and review verified 6/6 fresh.
```

## Risks

- Generated memory and archive material is demoted/hidden only for general queries
- explicit checkpoint/archive/security-research queries still surface it. Large dirty tree remains expected lab state.
