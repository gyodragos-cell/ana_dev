# Session Checkpoint - 2026-05-31T11:10:31+00:00

## Graph blast-radius query

## Summary

Added Graph Map blast-radius analysis through ana_graph_map.py action=blast and graph_context_pack action=blast. It estimates affected files/tests/docs from changed files, filters common context noise, downranks low-signal archive/checkpoint/report paths, and prioritizes probable tests.

## Current Goal

Give ANA stronger pre-edit impact awareness so Codex can choose targeted context and tests before changing code.

## Next Steps

- After MCP restart/reload, verify graph_context_pack action=blast live
- consider wiring blast-radius into ana_patch_advisor or autonomy_runner pre-edit planning.
...
```text
pytest test_ana_graph_map + test_graph_context_pack_tool PASS 6/6; governance PASS 111/111; no_reload_quality_gate PASS 7/7 report no_reload_quality_gate_20260531_141018.json; local blast command returned ana.graph_blast_radius.v1.
```

## Risks

- Live MCP needs restart/reload before graph_context_pack exposes action=blast. Blast quality depends on fresh Code Map/Graph Map and current code-map coverage.
