# Session Checkpoint - 2026-05-31T13:05:38+00:00

## Live behavior freshness check

## Summary

Added ana_live_behavior_check.py to detect healthy-but-stale MCP behavior by probing live session_audit action=trust for identity_surface fields. Wired live behavior into Post-Reload Verify and no-reload advisory checks. Updated examples, checklist, docs index, and AGENT_MEMORY.

## Current Goal

Make ANA MAX distinguish online MCP, matching tool surface, and current live tool behavior.

## Next Steps

- Restart MCP when convenient, then rerun ana_post_reload_verify.py --no-write. Expected after restart: tool_surface PASS and behavior PASS.
...
```text
live behavior/post-reload/gate contract pytest 12/12 PASS; governance PASS 115/115; no_reload_quality_gate PASS 8/8 with expected live_tool_surface and live_behavior advisories; lab_quality_gate PASS 10/10.
```

## Risks

- Live MCP still needs restart to load disk-side session_audit identity fields and 90-tool surface.
