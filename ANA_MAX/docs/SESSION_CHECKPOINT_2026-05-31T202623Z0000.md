# Session Checkpoint - 2026-05-31T20:26:23+00:00

## Operator Status precise MCP restart advice

## Summary

Refined ana_operator_status next_action so pure live_behavior_stale recommends restarting ANA MCP directly instead of the broader VSIX install/reload flow. Marker or tool-surface drift still keeps the VSIX/reload guidance.

## Current Goal

Make operator-facing reload guidance match the actual stale signal and avoid unnecessary extension work.

## Next Steps

- Restart ANA MCP when convenient, then rerun Live Behavior, Reload Consistency, Operator Status, and Autonomy Pass.
...
```text
operator/live/reload focused pytest 24 passed; lab_quality_gate PASS; no_reload_quality_gate PASS with expected live_behavior advisory WARN
```

## Risks

- Live MCP remains stale until restart
- no source/test failure is present.
