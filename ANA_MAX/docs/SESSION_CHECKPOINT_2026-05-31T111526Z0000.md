# Session Checkpoint - 2026-05-31T11:15:26+00:00

## Patch Advisor uses graph blast-radius

## Summary

Integrated local Graph Map blast-radius into ana_patch_advisor.py. Patch Advisor now reports blast_radius_available/candidates/affected, includes a suggest-only recommendation to review high-score affected files/tests, and avoids generic basename mismatches like tools/__init__.py -> plugins/__init__.py.

## Current Goal

Make ANA's self-healing lane more useful to Codex by combining diagnostics, git state, and graph impact analysis before patches.

## Next Steps

- After MCP reload, verify graph_context_pack action=blast live
- consider adding a pre-edit autonomy step that runs Patch Advisor or blast-radius when changed runtime files exist.
...
- docs/examples/GRAPH_BLAST_RADIUS_EXAMPLE.md

## Validation

```text
pytest graph+patch+graph_context focused PASS 15/15; governance PASS 111/111; no_reload_quality_gate PASS 7/7 report no_reload_quality_gate_20260531_141512.json; live ana_patch_advisor report includes blast_radius_available=true and suggest-only graph blast recommendation.
```

## Risks

- Live MCP still needs reload for graph_context_pack action=blast
- graph impact quality depends on fresh graph map and code map coverage.
