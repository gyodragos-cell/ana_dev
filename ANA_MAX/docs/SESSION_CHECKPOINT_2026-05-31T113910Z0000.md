# Session Checkpoint - 2026-05-31T11:39:10+00:00

## Autonomy output shows trace alignment

## Summary

Autonomy Runner human output now includes trace alignment in the header, e.g. trace=12/12 aligned=True, so operators see trace health without opening JSON or running ana_trace_report separately.

## Current Goal

Keep ANA's live operator feedback compact and immediately useful.

## Next Steps

- Continue with scoped reliability improvements
- possible next step is making Operator Status optionally run no-write trace validation when no trace report exists.
...
- Header trace alignment reflects report-local spans only
- deeper session_audit/event_stream merge remains future work.
