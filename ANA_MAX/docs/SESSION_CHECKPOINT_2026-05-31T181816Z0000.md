# Session Checkpoint - 2026-05-31T18:18:16+00:00

## VSIX 1.0.47 installed and reload readiness fixed

## Summary

Installed ANA MAX Codex Cockpit VSIX v1.0.47 locally. Did not force VS Code reload. Fixed ana_reload_readiness so it uses reload marker, live tool-surface drift, and live behavior staleness; live readiness now correctly reports reload_needed=True with reasons live_tool_surface_drift and live_behavior_stale.

## Current Goal

Complete the operator reload path for v1.0.47 without disrupting the active chat.

## Next Steps

- Operator should run Developer: Reload Window, restart ANA MCP, then run ANA MAX: Post-Reload Verify. Expected after restart: tools=90, tool_surface PASS, behavior PASS, identity PASS.
...
- installed vscode_extension/ana-codex-cockpit-1.0.47.vsix

## Validation

```text
VSIX install succeeded; reload readiness tests/operator/live-behavior tests PASS 19/19; reload readiness live says reload_needed=True with correct reasons; governance PASS 115/115; no_reload_quality_gate PASS 8/8 with expected live advisories; lab_quality_gate PASS 10/10.
```

## Risks

- VS Code still needs manual Developer: Reload Window
- MCP still needs restart to clear stale live tool surface and behavior.
