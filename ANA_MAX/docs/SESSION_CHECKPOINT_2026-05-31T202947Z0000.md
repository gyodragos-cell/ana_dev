# Session Checkpoint - 2026-05-31T20:29:47+00:00

## Post-Reload Verify restart guidance aligned

## Summary

Aligned Post-Reload Verify live_behavior_stale guidance with Operator Status. It now says to restart ANA MCP, then run Live Behavior, Reload Consistency, and Post-Reload Verify, avoiding vague rerun-only wording.

## Current Goal

Keep all operator-facing stale-live guidance consistent and action-specific.

## Next Steps

- Restart ANA MCP when convenient, then rerun Live Behavior, Reload Consistency, Operator Status, Post-Reload Verify, and Autonomy Pass.
...
```text
post-reload/operator/reload/live focused pytest 30 passed; lab_quality_gate PASS; no_reload_quality_gate PASS with expected live_behavior advisory WARN
```

## Risks

- Live MCP remains stale until restart
- behavior is intentionally WARN not FAIL.
