# Session Checkpoint - 2026-05-31T12:39:39+00:00

## Lab state shows tool-surface drift

## Summary

Added live tool-surface drift to Lab State Summary so the compact handoff shows disk-vs-live MCP mismatch and recommends post-reload verify when live tools/list is stale.

## Current Goal

Make every operator summary honest about live MCP drift after active tool surface changes.

## Next Steps

- Restart ANA MCP when ready
- then run post-reload verify and lab state summary. Expected target: tool_surface=PASS(live=90,manifest=90,extra=0,missing=0).
...
```text
lab/operator/post-reload tests PASS 15/15; live lab_state_summary shows tool_surface=WARN(live=91,manifest=90,extra=1,missing=0); lab_quality_gate PASS 9/9.
```

## Risks

- Live MCP still stale until restart
- lab state WARN is expected and useful.
