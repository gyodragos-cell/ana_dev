# Session Checkpoint - 2026-05-31T20:21:56+00:00

## Live behavior detects stale Error Radar

## Summary

Extended ana_live_behavior_check to compare live error_radar dirty-tree runtime counts against disk-side ErrorRadarTool behavior. This catches healthy-but-stale MCP servers after diagnostic classifier fixes.

## Current Goal

Make reload diagnostics detect stale live tool behavior, not just stale tool lists or session_audit fields.

## Next Steps

- Restart ANA MCP when convenient, then rerun Live Behavior, Reload Consistency, Operator Status, and Autonomy Pass to confirm all PASS.
...
```text
live behavior/reload/operator focused pytest 26 passed; no_reload_quality_gate PASS with expected live_behavior advisory WARN; lab_quality_gate PASS
```

## Risks

- Live MCP currently remains stale for error_radar until server restart
- this is expected and now correctly visible.
