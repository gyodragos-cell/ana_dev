...
- Cockpit VSIX `d4d8176a-bb85-66ef-93dd-a58bc9ddfdad.ana-antigravity-chat@1.0.9` is `ANA MAX - Hybrid AI Cockpit`. It includes the marketplace icon at `assets/ana-max-icon.png`, author credit `Dragos / gyodragos-cell with Codex`, public repo/homepage/license metadata, marketplace keywords, workspace-relative `runtimeRoot` defaults, Smart Ready, Checkpoint, REM Sleep, `tool_router`, and `agent_coach action=recommend` flows. Local marketplace artifact: `vscode_extension/ana-antigravity-chat-1.0.9.vsix`. Lab packaging also produced `ANA_MAX/ana-antigravity-hybrid-1.0.9.vsix`.
